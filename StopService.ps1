...
[parameter(position=0, mandatory=$true , HelpMessage = '停止対象のWindowsサービス名を指定(ex. spooler) 全てのHelpはGet-Help StopService.ps1')][String]$Service  ,
...
Logging -EventID $InfoEventID -EventType Information -EventMessage "サービス[$($Service)]の停止処理を開始します"
...
$Counter = 0
        ...
        $Return = $WMIService.stopService() 
           ...
           Switch ($Return.returnvalue)  {
                ...
                $ServiceStatus = CheckServiceStatus -ServiceName $Service -Health Stopped -Span $RetrySpanSec -UpTo $RetryTimes
                    ...
                    Logging -EventID $InfoEventID -EventType Information -EventMessage "サービス[$($Service)]は未だ正常に停止していません。再試行します"
                    }
    ...
    Logging -EventID $InfoEventID -EventType Information -EventMessage "サービス[$($Service)]が停止リクエストを受け付けません"
        ...
        Logging -EventID $ErrorEventID -EventType Error -EventMessage "指定期間、回数が経過しましたがサービス[$($Service)]が停止出来ませんでした"
        Finalize $ErrorReturnCode
        }

      #チェック回数の上限に達していない場合は、指定秒待機

      Logging -EventID $InfoEventID -EventType Information -EventMessage "サービス[$($Service)]は存在しますが停止できませんでした。$($RetrySpanSec)秒待機します。"
